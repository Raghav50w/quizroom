 ...
 *
 * Imports nothing from src/server or src/client — a PDF's retrieved excerpt
 * feeds this same pipeline, and the folder could be lifted into its own repo.
 */
...
export async function generateQuiz(options: GenerateOptions): Promise<GenerateResult> {
  ...
  const raw = await requestQuestions(source, asked, log, options.prompt);
  ...
  const result: GenerateResult = { quiz };
  if (selected.length < count) {
    result.shortfall = { requested: count, delivered: selected.length };
  }
  return result;
}
